          ...
          <div className="bg-gradient-to-br from-neutral-900/50 to-neutral-800/30 rounded-2xl p-6 border border-cyan-500/20">
            <div className="flex items-center justify-between mb-4">
              ...
              {!gameActive && (
                <button
                  onClick={startGame}
                  className="flex items-center space-x-2 px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white font-semibold rounded-lg transition-colors"
                >
            ...
            {gameActive ? (
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <div className="flex space-x-6">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-cyan-400">{gameScore}</div>
                      <div className="text-xs text-gray-400">Score</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-blue-400">{gameTime}</div>
                      <div className="text-xs text-gray-400">Time</div>
                    </div>
                  </div>
                  <button
                    onClick={stopGame}
                    className="flex items-center space-x-2 px-3 py-1 bg-red-600 hover:bg-red-500 text-white text-sm font-semibold rounded transition-colors"
                  >
                ...
                <div className="text-center">
                  <div className="text-3xl font-bold text-cyan-400 mb-2">{currentWord}</div>
                  <input
                    type="text"
                    value={userInput}
                    onChange={handleInputChange}
                    className="w-full max-w-md px-4 py-2 bg-neutral-800 border border-cyan-500/30 rounded-lg text-white text-center text-lg focus:outline-none focus:border-cyan-400"
            ...
            {blogs.length === 0 ? (
              <div className="col-span-full text-center py-12">
                <div className="bg-gradient-to-br from-neutral-900/50 to-neutral-800/30 rounded-2xl p-8 border border-cyan-500/20">
                  <FaBlog className="text-4xl text-cyan-400 mx-auto mb-4" />
                  <h3 className="text-xl font-semibold text-cyan-400 mb-2">No Blogs Yet</h3>
              ...
              blogs.map((blog, index) => (
                <div
                  key={blog.id}
                  className="group bg-gradient-to-br from-neutral-900/50 to-neutral-800/30 border border-cyan-500/20 rounded-2xl shadow-2xl p-6 hover:border-cyan-400/40 transition-all duration-300 hover:scale-105 flex flex-col"
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      {/* Blog title */}
                      <h3 className="text-xl font-bold text-cyan-400 mb-2 group-hover:text-cyan-300 transition-colors line-clamp-2">
                        {blog.title}
                      </h3>
                    ...
                    <div className="p-2 bg-cyan-500/20 rounded-lg group-hover:bg-cyan-500/30 transition-colors">
                      <FaBlog className="text-cyan-400 text-lg group-hover:rotate-12 transition-transform duration-300" />
                    </div>
                  </div>
